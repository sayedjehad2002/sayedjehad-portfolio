  ...
  return (
    <div className="anim-fade absolute inset-0 z-10 flex flex-col px-4 py-3" onKeyDown={onKey}>
      <div className="flex items-start justify-between">
        <button
          onClick={onBack}
          className="ui-focus-dark inline-flex min-h-[44px] items-center rounded px-3 font-pixel text-[9px] uppercase tracking-wider text-[#9fb0c0] outline-none transition-colors hover:text-[#ffffff]"
        >
        ...
        <div className="text-center">
          <h2 className="font-pixel text-[14px] uppercase tracking-wider text-[#FFE08A] [text-shadow:0_0_8px_rgba(255,224,138,0.4)]">Select Game</h2>
          <div className="font-pixel text-[8px] uppercase tracking-[0.3em] text-[#5FE0D0]" aria-hidden="true">
            Choose your cartridge
          </div>
        </div>
        <div className="text-right font-pixel text-[8px] uppercase leading-tight" aria-hidden="true">
      ...
      <div className="flex flex-1 items-center justify-center gap-3 sm:gap-4">
        {games.map((g, i) => (
          <div key={g.id} className="flex flex-col items-center">
            <span
              className={'mb-1 font-pixel text-[13px] text-[#FF5A4D] [text-shadow:0_0_6px_rgba(255,90,77,0.7)] ' + (sel === i ? 'opacity-100' : 'opacity-0')}
              aria-hidden="true"
            >
              ▾
            </span>
            ...
            >
              <div className="px-2 py-1 font-pixel text-[8px] uppercase tracking-wider text-[#04130d]" style={{ background: g.accent }}>
                {g.name}
              </div>
              <div className="flex flex-col items-center gap-1.5 px-2 py-3">
                ...
                <div className="font-pixel text-[12px] text-[#FFE08A]">{g.name}</div>
                <div className="min-h-[26px] font-sans text-[10.5px] leading-snug text-[#9fb0c0]">{g.tagline}</div>
                <div className="font-pixel text-[9px] uppercase tracking-wider" style={{ color: g.accent }}>
                  Best {pad5(g.best())}
                </div>
              </div>
            </button>
      ...
      <div className="text-center font-pixel text-[8px] uppercase tracking-[0.2em] text-[#6c7d8f]" aria-hidden="true">
