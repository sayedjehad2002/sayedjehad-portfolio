...
const ROWS: Array<{ key: BoolKey; label: string }> = [
  ...
  { key: 'oceanEnabled', label: 'Sea life' },
...
export function EnvSettings() {
  ...
  return (
    <div className="mt-5 border-t border-line/70 pt-4">
      <span className="font-pixel text-eyebrow uppercase tracking-widest text-teal-deep">Atmosphere</span>
        ...
        {ROWS.map((r) => {
          ...
          return (
            <li key={r.key}>
              ...
              >
                <span className="font-sans text-body text-ink-soft">{r.label}</span>
      ...
      <div className="flex min-h-[44px] items-center justify-between gap-3 px-1">
        <span className="font-sans text-body text-ink-soft">Weather</span>
        <button
          type="button"
          onClick={() => {
          ...
          className="ui-focus-panel rounded-lg border border-line bg-sunken px-3 py-1.5 font-sans text-small font-semibold text-teal-deep outline-none transition-colors hover:border-teal"
        >
