...
export function TechPanel() {
  return (
    <div>
      <span className="font-pixel text-eyebrow uppercase tracking-widest text-teal-deep">Toolkit</span>
      <h2 id="modal-title" className="mb-2 mt-2 font-sans text-display font-bold leading-tight text-ink">
        Production stack
      </h2>
      <p className="mb-4 font-sans text-ui leading-relaxed text-ink-soft">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {STACK.map((t) => (
          ...
          >
            <div className="font-sans text-small font-bold text-ink">{t.name}</div>
            <div className="font-sans text-caption text-ink-faint">{t.desc}</div>
          </div>
        ))}
      </div>
    </div>
